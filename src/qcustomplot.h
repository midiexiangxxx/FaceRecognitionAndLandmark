...
class QCP_LIB_DECL QCPLayerable : public QObject
{
  ...
  /// \endcond
public:
  QCPLayerable(QCustomPlot *plot, QString targetLayer=QString(), QCPLayerable *parentLayerable=NULL);
  ...
  // introduced virtual methods:
  virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details=NULL) const;
...
public:
  ...
  */
  ...
  explicit QCPLayoutElement(QCustomPlot *parentPlot=NULL);
  ...
  QCPMarginGroup *marginGroup(QCP::MarginSide side) const { return mMarginGroups.value(side, NULL); }
  ...
  // reimplemented virtual methods:
  virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details=NULL) const Q_DECL_OVERRIDE;
...
protected:
  ...
  double getMantissa(double input, double *magnitude=NULL) const;
...
public:
  ...
  // introduced virtual methods:
  virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details=NULL) const Q_DECL_OVERRIDE = 0; // actually introduced in QCPLayerable as non-pure, but we want to force reimplementation for plottables
  virtual QCPPlottableInterface1D *interface1D() { return NULL; }
...
protected:
  ...
  // introduced virtual methods:
  virtual QCPItemPosition *toQCPItemPosition() { return NULL; }
...
public:
  ...
  // reimplemented virtual methods:
  virtual double selectTest(const QPointF &pos, bool onlySelectable, QVariant *details=NULL) const Q_DECL_OVERRIDE = 0;
...
public:
  ...
  */
  ...
  explicit QCustomPlot(QWidget *parent = NULL);
  ...
  PlottableType *plottableAt(const QPointF &pos, bool onlySelectable=false, int *dataIndex=NULL) const;
  QCPAbstractPlottable *plottableAt(const QPointF &pos, bool onlySelectable=false, int *dataIndex=NULL) const;
  ...
  bool addLayer(const QString &name, QCPLayer *otherLayer=NULL, LayerInsertMode insertMode=limAbove);
...
protected:
  ...
  QCPLayerable *layerableAt(const QPointF &pos, bool onlySelectable, QVariant *selectionDetails=NULL) const;
  QList<QCPLayerable*> layerableListAt(const QPointF &pos, bool onlySelectable, QList<QVariant> *selectionDetails=NULL) const;
...
/*!
  ...
  If there is no plottable of the specified type at \a pos, returns \c NULL.
...
PlottableType *QCustomPlot::plottableAt(const QPointF &pos, bool onlySelectable, int *dataIndex) const
{
  ...
  foreach (QCPAbstractPlottable *plottable, mPlottables)
  {
    ...
    {
      QVariant details;
      double currentDistance = currentPlottable->selectTest(pos, false, dataIndex ? &details : NULL);
...
/*!
  ...
  If there is no item at \a pos, returns \c NULL.
...
public:
  ...
  QCPAxis *addAxis(QCPAxis::AxisType type, QCPAxis *axis=NULL);
...
protected:
  ...
  QRect getFinalRect(bool *flippedHorz=NULL, bool *flippedVert=NULL) const;
  QPen mainPen() const;
};
